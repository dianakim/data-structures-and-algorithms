...
const listFoods = (recipe) => {
  ...
  // then grab everything after that and push to the result array
...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let splitDetails = recipe.ingredients[i].split(' ');
    splitDetails.splice(0, 2);
    result.push(splitDetails.join(' '));
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  str.splice(str.length - numberOfCharacters, str.length - 1);
  return str;
};
