...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let totalElements = arr.reduce( (total, element, index) => {
...
const reversedString = (str) => {
  ...
  let backwards = strArr.reduce( (newString, currentCharacter) => {
    return currentCharacter + newString;
  }, '');
  return backwards;
};
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let totalChildren = arr.reduce( (total, currentCharacter) => {
    if (currentCharacter.children) {
      total = total + currentCharacter.children.length;
    }
    return total;
  }, 0);
...
const countPrimeNumbers = (arr) => {
  // Solution code here...
  let totalNumOfPrime = arr.reduce( (count, currentNumber) => {
    if (isPrime(currentNumber)) {
      count++;
    };
    return count;
  }, 0);
  return totalNumOfPrime;
};
...
const extractStat = (statName, arr) => {
  // Solution code here...
  let match = arr.reduce( (matchingObjects, currentObject, idx) => {
...
const extractChildren = (arr) => {
  // Solution code here...
  let namesWithA = arr.filter( (currentCharacter, idx) => {
    let nameArray = (currentCharacter.name.toLowerCase()).split('');
